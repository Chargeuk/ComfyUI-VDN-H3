...
def frame_statistics(kf, vf, beta, a_fp32=True, fuse=False):
    """Bound preparation memory by batching complete, independent frames."""
    ...
    if frames <= batch:
        return _frame_statistics_chunk(kf, vf, beta, a_fp32, fuse=fuse)
    ...
    for start in range(0, frames, batch):
        ...
        ac, bc = _frame_statistics_chunk(kf[start:stop], vf[start:stop],
                                         beta[start:stop], a_fp32, fuse=fuse)
...
def _frame_statistics_chunk(kf, vf, beta, a_fp32=True, fuse=False):
